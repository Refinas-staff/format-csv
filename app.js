(function () {
  ...
  function renderPatternInfo() {
    ...
    if (!pattern) return;
    const inputItems = pattern.inputHeaders.map(h => `<li><code>${escapeHtml(h)}</code></li>`).join("");
    const ruleItems = (pattern.rules || []).map(r => `<li>${escapeHtml(r)}</li>`).join("");
  ...
  async function convert() {
    const pattern = getPattern();
    if (!pattern) return setStatus("整形パターンを選択してください。", "error");
    if (!state.file) return setStatus("CSVファイルを選択してください。", "error");
    ...
    try {
      ...
      const rows = parsed.rows.map(row => rowObject(headers, row)).filter(row => Object.values(row).some(v => String(v).trim() !== ""));
      ...
      if (pattern.type === "row") {
        ...
        rows.forEach(row => {
          const converted = pattern.transform(row);
          tableRows.push(pattern.outputHeaders.map(header => converted[header] ?? ""));
        });
        state.result = {
          type: "table",
          fileBaseName: pattern.id,
          sheets: [{ name: pattern.name, rows: tableRows, styleMatrix: defaultStyleMatrix(tableRows) }]
        };
      } else if (pattern.type === "custom") {
        const customResult = pattern.transformAll(rows);
        state.result = {
          type: "workbook",
          fileBaseName: pattern.id,
      ...
      setStatus(`整形が完了しました。\n読み込み件数: ${rows.length}件\n出力シート数: ${state.result.sheets.length}${warningText}`, "success");
    } catch (error) {
      console.error(error);
  ...
  async function readFileText(file, encoding) {
    ...
    if (replacementCount > 2) return new TextDecoder("shift_jis").decode(bytes);
    return utf8Text;
  }
  ...
  function parseDelimitedText(text) {
    ...
    return { headers: rows[0], rows: rows.slice(1), delimiter };
  }
  ...
  function defaultStyleMatrix(rows) {
    return rows.map((row, rowIndex) => row.map(() => rowIndex === 0 ? { fill: "E5E7EB", bold: true } : { fill: "FFFFFF" }));
  }
  ...
  function renderPreview() {
    ...
    if (!sheet) return renderEmptyPreview();
    ...
    $("previewNote").textContent = sheet.rows.length > maxRows ? `先頭${maxRows}行を表示しています。編集内容は出力に反映されます。` : "セルは直接編集できます。編集内容は出力に反映されます。";
  }

  function applyPreviewStyle(element, style) {
    if (!style) return;
    if (style.fill) element.style.backgroundColor = `#${style.fill}`;
    if (style.bold) element.style.fontWeight = "900";
  }
  ...
  function applyWorksheetStyles(ws, sheet) {
    ...
    for (let r = range.s.r; r <= range.e.r; r++) {
      for (let c = range.s.c; c <= range.e.c; c++) {
        ...
        const style = sheet.styleMatrix && sheet.styleMatrix[r] && sheet.styleMatrix[r][c] ? sheet.styleMatrix[r][c] : {};
  ...
  function toXlsxStyle(style) {
    return {
      fill: { patternType: "solid", fgColor: { rgb: style.fill || "FFFFFF" } },
      font: { bold: !!style.bold, name: "Meiryo" },
      border: {
  ...
  function autoColumns(rows) {
    ...
    return Array.from({ length: maxCols }, (_, c) => {
      ...
      return { wch: Math.min(Math.max(max + 2, 8), 24) };
    });
  }
